    ...
    >
      <div className="mx-auto grid max-w-[1440px] gap-14 px-6 sm:px-10 lg:grid-cols-12 lg:gap-16 lg:px-14">
        <div className="lg:col-span-5">
          <h2 className="max-w-[11ch] text-[clamp(2.65rem,4.6vw,4.75rem)] font-medium leading-[0.96] tracking-[-0.05em] text-[#1A1A18] text-balance">
          ...
          <div className="mt-10 flex items-center gap-4">
            <strong className="text-5xl font-medium tracking-[-0.06em] text-[#B74716] tabular-nums sm:text-6xl">
              100%
            </strong>
            <span className="max-w-28 text-xs font-semibold uppercase leading-relaxed tracking-[0.16em] text-[#626760]">
        ...
        <div className="grid gap-x-10 sm:grid-cols-2 lg:col-span-7">
          {reasons.map((reason) => {
            ...
            return (
              <article
                key={reason.title}
                className="group border-t border-[#DCDDD6] py-7 sm:min-h-64 sm:py-8"
              >
                <Icon
                  size={30}
                  weight="light"
                  className="text-[#B74716] transition-transform duration-300 group-hover:-translate-y-1 motion-reduce:transform-none"
                  aria-hidden="true"
                />
                <h3 className="mt-8 text-xl font-medium tracking-[-0.03em] text-[#1A1A18] sm:text-2xl">
                  {reason.title}
                </h3>
                <p className="mt-3 max-w-sm text-sm font-normal leading-relaxed text-[#626760] sm:text-base">
                  {reason.description}
                </p>
              </article>
            );
